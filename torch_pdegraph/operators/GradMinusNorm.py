...
class OPE(MessagePassing):
    """
    Computes:
    \|(\nabla_{w}^{-}f(v_{i})\|_{p} = \left [ \sum_{v_{j}\sim v_{i}} w(v_{i}v_{j})^{\frac{p}{2}}((f(v_j) - f(v_i))^{-})^{p}  \right]^{\frac{1}{p}}
    """
    ...
    def message(self, x_i, x_j):
        return torch.pow(torch.sqrt(self.edge_attr)  * torch.clamp(x_i-x_j, 0), self.p)
